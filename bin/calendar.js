...
Calendar.prototype = {
    addDaysToDate: function addDaysToDate(date, days) {
        return new Date(date.getFullYear(), date.getMonth(), date.getDate() + days);
    },

    getDaysInMonth: function getDaysInMonth(mo, yr) {
        mo = mo || undefined.now.getMonth();
        yr = yr || undefined.now.getFullYear();
        return 32 - new Date(yr, mo - 1, 32).getDate();
    },

    getLastDayOfWeek: function getLastDayOfWeek(day) {
        if (day > 0) {
            return day;
        } else {
            return 6;
    ...
    getRelativeMonth: function getRelativeMonth(date, dif) {
        var curIndex = date.getMonth();
        var newIndex = curIndex + dif;

        if (newIndex < 0) newIndex = 11;else if (newIndex > 11) newIndex = 1;
